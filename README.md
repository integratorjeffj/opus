...
**[Click through the app in your browser](https://integratorjeffj.github.io/opus/demo/)**
— the demo-data gate, a day of PayPal orders, the review screen that flags the
piece it could not match, and the ledger, ending on a real stamped page.

It is a simulation of the desktop app, and it says so: nothing is uploaded and
no PDF is produced in your browser. Every number, order reference, filename and
ledger row in it is the actual output of running Opus against the fictional
catalogue in this repository, generated by `packaging/build_demo.py` so it
cannot drift from what the tool really does.
